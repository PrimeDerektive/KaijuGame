...
public class WaveEmitter : MonoBehaviour {
	...
	void EmitWaves() {
		...
		for(int i=0;i<particles.Length;i++) {
			...
			particles[i].position = new Vector3(particle.position.x,UTW.getHeightByPos(particle.position)+0.75f,particle.position.z);
		}
		...
		if (particleDiff > 0) {
			...
			for (int i = 0; i < particleDiff; i++ ) {
				...
				Vector2 randPos = Random.insideUnitCircle.normalized * Random.Range(100f,Mathf.Max(UTW.config.size.x,UTW.config.size.y)/2f);
