	...
	void Update () {
		...
		if (snapToWater == true) {
			...
			for(int i=0;i<particles.Length;i++) {
				...
				if (UTW != null) {
					particles[i].position = new Vector3(particle.position.x,UTW.getHeightByPos(particle.position)+1.25f,particle.position.z);
				} else if (UTWC != null) {
					particles[i].position = new Vector3(particle.position.x,UTWC.getHeightByPos(particle.position)+1.25f,particle.position.z);
				}
			}
