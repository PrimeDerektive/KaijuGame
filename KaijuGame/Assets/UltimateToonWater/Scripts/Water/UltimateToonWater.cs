...
public class UltimateToonWater : MonoBehaviour {
	...
	public class WaveForm {
		public Vector2 speed;			// Wave travel speed (Meters/Second)
		public Vector2 waveLength;		// Wavelength (Peak-to-Peak) (Meters)
		public float scale;				// Waveheight (Meters)
	...
	void Awake() {
		GetComponent<Renderer>().sharedMaterial = new Material(baseMaterial);
		generateMesh();
		UpdateColor();
		UpdateTextures();
		UpdateTextureProps();
		...
		if (size > 0 && meshPointDistance > 0 && size > (meshPointDistance*2)) {
			...
			for (float y = 0.0f; y < vCount2; y++)
			{
				for (float x = 0.0f; x < hCount2; x++)
				{
					...
					vertices[index] = new Vector3(x*scaleX - size/2f - anchorOffset.x, 0, y*scaleY - size/2f - anchorOffset.y);
	...
	public void UpdateTextureSize() {
		if (size > 0 && mainTextureTitle.x > 0 && mainTextureTitle.y > 0 && detailTextureTitle.x > 0 && detailTextureTitle.y > 0) {
			gameObject.GetComponent<Renderer>().sharedMaterials[0].SetTextureScale(mainTextureName,new Vector2(size/mainTextureTitle.x,size/mainTextureTitle.y));
			gameObject.GetComponent<Renderer>().sharedMaterials[0].SetTextureScale(detailTextureName,new Vector2(size/detailTextureTitle.x,size/detailTextureTitle.y));
		}
	}

	public void UpdateColor() {
		gameObject.GetComponent<Renderer>().sharedMaterials[0].SetColor("_Color",waterColor);
		gameObject.GetComponent<Renderer>().sharedMaterials[0].SetColor("_Color2",waveColor);
	}

	public void UpdateTextures() {
		gameObject.GetComponent<Renderer>().sharedMaterials[0].SetTexture(mainTextureName,mainTexture);
		gameObject.GetComponent<Renderer>().sharedMaterials[0].SetTexture(detailTextureName,detailTexture);
	}

	public void UpdateTextureProps() {
		gameObject.GetComponent<Renderer>().sharedMaterials[0].SetFloat("_ColorBias",colorTextureRatio);
		gameObject.GetComponent<Renderer>().sharedMaterials[0].SetFloat("_HeightFactor",heightColoring);
		gameObject.GetComponent<Renderer>().sharedMaterials[0].SetFloat("_HighlightThresholdMax",shoreFoamLine);
		gameObject.GetComponent<Renderer>().sharedMaterials[0].SetFloat("_HighlightThresholdRange",shoreFoamCut);
		gameObject.GetComponent<Renderer>().sharedMaterials[0].SetFloat("_TextureBias",textureBias);
	}
	...
	public float getHeightByPos(Vector2 pos) {
		...
		return offset+transform.position.y+2;
	}
