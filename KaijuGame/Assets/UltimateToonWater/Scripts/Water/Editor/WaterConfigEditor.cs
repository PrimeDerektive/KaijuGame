...
public class WaterConfigEditor : Editor {
	...
	void OnEnable () {
		...
		vSize = dimObj.FindProperty("size");
	...
	public override void OnInspectorGUI ()
	{
		...
		if (openBasic == true) {
			Vector2 vSizeOld = vSize.vector2Value;
			float fMeshPointDistanceOld = fMeshPointDistance.floatValue;
			
			vSize.vector2Value = EditorGUILayout.Vector2Field("Square Water Size: ",vSize.vector2Value);
			fMeshPointDistance.floatValue = EditorGUILayout.FloatField("Meshpoint Distance: ",fMeshPointDistance.floatValue);
			if (vSizeOld != vSize.vector2Value || fMeshPointDistanceOld != fMeshPointDistance.floatValue) {
