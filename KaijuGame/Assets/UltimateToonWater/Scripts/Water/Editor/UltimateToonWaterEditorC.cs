...
public class UltimateToonWaterCEditor : Editor { 
	...
	public override void OnInspectorGUI ()
	{
		...
		if (config.objectReferenceValue != null && safe == true) {
			...
			if (openBasic == true) {
				Vector2 fSizeOld = waterConfig.size;
				float fMeshPointDistanceOld = waterConfig.meshPointDistance;
				
				waterConfig.size = EditorGUILayout.Vector2Field("Square Water Size: ",waterConfig.size);
		...
		if (needGenerateMesh == true) {
			//int numVerts = (int) (waterConfig.size/waterConfig.meshPointDistance);
			int numVerts = (int) ((waterConfig.size.x/waterConfig.meshPointDistance)*(waterConfig.size.y/waterConfig.meshPointDistance));
			//numVerts *= numVerts;
			if (numVerts > 65000) {
				waterConfig.meshPointDistance = Mathf.CeilToInt(Mathf.Max(waterConfig.size.x,waterConfig.size.y) / Mathf.Sqrt(65000f));
				dimObj.ApplyModifiedProperties();
			}
