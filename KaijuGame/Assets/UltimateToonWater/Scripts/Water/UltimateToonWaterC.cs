...
public class UltimateToonWaterC : MonoBehaviour {
	...
	public void generateMesh() {
		if (config.size.x > 0 && config.size.y > 0 &&  config.meshPointDistance > 0 && config.size.x > (config.meshPointDistance*2) && config.size.y > (config.meshPointDistance*2)) {
			int xTitle = (int)(config.size.x/ config.meshPointDistance);
			int yTitle = (int)(config.size.y/ config.meshPointDistance);
			int numVerticesHalf = (int)(xTitle+yTitle)/2;
			m = new Mesh();
			m.MarkDynamic();
			
			int hCount2 = xTitle*2+1;
			int vCount2 = yTitle*2+1;
			int numTriangles = xTitle*yTitle*12;
			...
			int index = 0;
			float uvFactorX = 1.0f/xTitle;
			float uvFactorY = 1.0f/yTitle;
			float scaleX = config.size.x/xTitle;
			float scaleY = config.size.y/yTitle;
			for (float y = 0.0f; y < vCount2; y++)
			{
				for (float x = 0.0f; x < hCount2; x++)
				{
					vertices[index] = new Vector3(x*scaleX - config.size.x/2f - anchorOffset.x, 0.0f, y*scaleY - config.size.y/2f - anchorOffset.y);
					uvs[index++] = new Vector2(x*uvFactorX, y*uvFactorY);
				}
			}
			
			index = 0;
			for (int y = 0; y < yTitle; y++)
			{
				for (int x = 0; x < xTitle; x++)
				{
	...
	public void UpdateTextureSize() {
		if (config != null && config.size.x > 0 &&  config.size.y > 0 && config.mainTextureTitle.x > 0 && config.mainTextureTitle.y > 0 && config.detailTextureTitle.x > 0 && config.detailTextureTitle.y > 0) {
			if (Application.isPlaying) {
				GetComponent<Renderer>().materials[0].SetTextureScale(mainTextureName,new Vector2(config.size.x/config.mainTextureTitle.x,config.size.y/config.mainTextureTitle.y));
				GetComponent<Renderer>().materials[0].SetTextureScale(detailTextureName,new Vector2(config.size.x/config.detailTextureTitle.x,config.size.y/config.detailTextureTitle.y));
			} else {
				GetComponent<Renderer>().sharedMaterials[0].SetTextureScale(mainTextureName,new Vector2(config.size.x/config.mainTextureTitle.x,config.size.y/config.mainTextureTitle.y));
				GetComponent<Renderer>().sharedMaterials[0].SetTextureScale(detailTextureName,new Vector2(config.size.x/config.detailTextureTitle.x,config.size.y/config.detailTextureTitle.y));
			}
		}
	}
	...
	public float getHeightByPos(Vector2 pos) {
		...
		return offset+transform.position.y;
	}
