	...
	void FixedUpdate () {
		...
		foreach(Floater floater in floaters) {
			...
			if (forceFactor > 0f) {
				...
				float buoyance = forceFactor*forceFactor*floater.buoyanceForce+floater.buoyanceForce;
				Vector3 uplift = -Physics.gravity * buoyance * speedFactor;
				uplift.y -= speed.y*forwardDrag;
				GetComponent<Rigidbody>().AddForceAtPosition(uplift, actionPoint,ForceMode.Force);
				if (drawGizmos == true) {
