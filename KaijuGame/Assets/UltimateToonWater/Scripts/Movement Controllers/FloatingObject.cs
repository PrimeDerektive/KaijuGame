	...
	void FixedUpdate () {
		if (isActive == true && (UTW != null || UTWC != null)) {
			...
			foreach(Floater floater in floaters) {
				...
				forceFactor = targetYPos - actionPoint.y;
				forceFactor = Mathf.Min (forceFactor,2.25f);
				
				if (forceFactor > 0f) {
					buoyance = forceFactor*forceFactor*floater.buoyanceForce+floater.buoyanceForce;
					Vector3 uplift = -Physics.gravity * buoyance;
					rb.AddForceAtPosition(uplift, actionPoint,ForceMode.Force);
					rb.AddTorque(-uplift);
					if (drawGizmos == true) {
					...
					buoyance = (waterDownDragY-forceFactor)*floater.buoyanceForce;
					Vector3 uplift = -Physics.gravity * buoyance;
					rb.AddForceAtPosition(uplift, actionPoint,ForceMode.Force);
