	...
	void Update()
    {
        ...
        if(ShootMode == Mode.ExplodeRaycast)
        {
            ...
            if(chunkRaycast)
            {
                ...
                {
                    // Hit it!
					FracturedObject objectHit = chunkRaycast.transform.parent.GetComponent<FracturedObject>();
					objectHit.Explode(hitInfo.point, 5000.0f);
					//Debug.Log(chunkRaycast.transform.parent);
                    //chunkRaycast.Impact(hitInfo.point, ExplosionForce, ExplosionRadius, false);
                }
            }
        }
